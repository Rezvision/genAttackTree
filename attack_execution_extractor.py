...
def create_graph_xml(parent_id, parent_name, attack_id, attack_name, children, execution_steps, output_file):
    root = ET.Element('sandtree')
    parent_node = ET.SubElement(root, 'node', refinement='disjunctive')
    parent_label = ET.SubElement(parent_node, 'label')
    parent_label.text = f"{parent_name} (ID: {parent_id})"

    main_node = ET.SubElement(parent_node, 'node', refinement='disjunctive')
...
def main(input_file, attack_id):
    ...
    if parent_id and parent_name:
        ...
        attack_pattern = tree.getroot().find(f".//capec:Attack_Pattern[@ID='{attack_id}']", {'capec': 'http://capec.mitre.org/capec-3'})
        attack_name = attack_pattern.attrib.get('Name', 'Unknown')
        create_graph_xml(parent_id, parent_name, attack_id, attack_name, children, execution_steps, output_file)
