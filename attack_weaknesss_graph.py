...
def create_dag_xml_tree(attack_pattern_id, attack_pattern_name, related_weaknesses):
    root = ET.Element("DAG")
    parent_nodes = []

    # Create parent nodes for each weakness ID
    for weakness_id in related_weaknesses:
        parent_node = ET.SubElement(root, "node", id=f"Weakness_{weakness_id}")
        label = ET.SubElement(parent_node, "label")
        label.text = f"{weakness_id}"
        parent_nodes.append(parent_node)

    # Create a child node for the attack pattern
    child_node = ET.SubElement(root, "node", id=f"Attack_{attack_pattern_id}")
    child_label = ET.SubElement(child_node, "label")
    child_label.text = f"{attack_pattern_name} (ID: {attack_pattern_id})"

    # Link each parent node to the child node
    for parent_node in parent_nodes:
        link = ET.SubElement(root, "link")
        source = ET.SubElement(link, "source")
        source.text = parent_node.get("id")
        target = ET.SubElement(link, "target")
        target.text = child_node.get("id")
...
def main(input_file, attack_pattern_id):
    ...
    if related_weaknesses:
        new_tree = create_dag_xml_tree(attack_pattern_id, attack_pattern_name, related_weaknesses)
        file_name = f"Attack_{attack_pattern_id}.xml"
